...
class Package extends CI_Controller {
	...
	public function index()
	{
		...
		$data = array();
		$data['package'] = $this->common_model->get_data_array(PACKAGE,$where);
		if(@$data['package']){ foreach($data['package'] as $k=>$v){
			$data['package'][$k]['ex_tax'] = $this->common_model->get_data_array(PACKAGE_TO_TAX,array('package_id'=>$v['package_id']));
		}}
		//echo $this->db->last_query();die;
		$data['pageTitle'] = "SCN | Internet | PACKAGE";
		...
		$this->load->view('internet/package', $data);
	}
	...
	public function add($id = null){
		if($this->input->post()){
			...
			$insert_array['staff_base'] = $this->input->post('tax_type1');
			...
			if(@$id){
				$this->common_model->tbl_update(PACKAGE,array('package_id'=>$id),$insert_array);
			}else{
				$id = $this->common_model->tbl_insert(PACKAGE,$insert_array);
			}
			$package_code = 'PAC000'.$id;
			$this->common_model->tbl_update(PACKAGE,array('package_id'=>$id),array('pakcode'=>$package_code));//DIE;
			//insert to package_to_tax table
			$this->common_model->tbl_record_del(PACKAGE_TO_TAX,array('package_id'=>$id));
			if(@$this->input->post('tax_name') && @$this->input->post('tax_no') && @$this->input->post('tax_type') && @$this->input->post('tax_price') && @$this->input->post('tax_type1')==2){
				...
				foreach($this->input->post('tax_name') as $k=>$v){
					...
					$this->common_model->tbl_insert(PACKAGE_TO_TAX,$insert_array1);
				}
			}
			$this->utilitylib->setMsg(SUCCESS_ICON.'Data successfully saved.','SUCCESS');
			redirect(base_url('internet/package'));
		}
		if(@$id){
			$data['details'] = $this->common_model->get_data_row(PACKAGE,array('package_id'=>$id));
			$data['package_tax'] = $this->common_model->get_data_array(PACKAGE_TO_TAX,array('package_id'=>$id));
		}
		$data['pageTitle'] = "SCN | Internet | Add Package";
		$data['header_links'] = $this->load->view('internet/includes/header_links',$data,true);
		$data['topbar'] = $this->load->view('internet/includes/topbar',$data,true);
		$data['left_menu'] = $this->load->view('internet/includes/left_menu','',true);
		$data['footer'] = $this->load->view('internet/includes/footer','',true);
		$data['footer_scripts'] = $this->load->view('internet/includes/footer_scripts','',true);
		$this->load->view('internet/package_add', $data);
	}
	...
	public function delete_record($id=null){
		...
		$this->common_model->tbl_record_del(PACKAGE,array('package_id'=>$id));
	...
	public function change_status($id=null){
		...
		$status = $this->common_model->get_data_row(PACKAGE,array('package_id'=>$id));
		if($status['status']==1){
			$this->common_model->tbl_update(PACKAGE,array('package_id'=>$id),array('status'=>2));	
		}else if($status['status']==2){
			$this->common_model->tbl_update(PACKAGE,array('package_id'=>$id),array('status'=>1));
		}
